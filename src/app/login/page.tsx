...
export default function Home() {
  ...
  const onHandlePasswordReset = async (e: FormEvent) => {
    e.preventDefault();
      ...
      <main className="relative z-[1] bg-[url(/images/grad_bg.jpg)] text-gray-200 min-h-screen flex flex-col items-center justify-center font-mono px-5 py-10 md:py-0">
        ...
        >
          ...
          <h2 className="text-4xl font-bold mt-2 text-white">
            {activeSection == "login" ? "Login" : "Password Reset"}
          </h2>
        </motion.header>
        <motion.div
          className="max-w-md w-full space-y-6 relative min-h-[270px]"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          exit={{ y: 20, opacity: 0 }}
          transition={{ duration: 0.6, delay: introAniDelay + 0.15 }}
        >
          <LoginSection section={activeSection}>
            {/* LOGIN FORM */}
            {activeSection == "login" ? (
              <form onSubmit={handleSubmit} className="w-full">
                ...
                <button
                  type="submit"
                  disabled={processing}
                  className="w-full bg-amber-500 hover:bg-amber-600 disabled:bg-amber-700 disabled:cursor-not-allowed text-black font-bold py-3 px-4 rounded-lg transition-colors duration-200 my-5"
                >
                ...
                <div className="text-center">
                  <a
                    ...
                    onClick={(e) => updateSection(e, "recover")}
                  >
            ...
            {activeSection == "recover" ? (
              <form onSubmit={onHandlePasswordReset} className="w-full">
                <div className="space-y-4">
                  <div>
                    <label
                ...
                <button
                  type="submit"
                  disabled={processing}
                  className="w-full my-5 bg-amber-500 hover:bg-amber-600 disabled:bg-amber-700 disabled:cursor-not-allowed text-black font-bold py-3 px-4 rounded-lg transition-colors duration-200"
                >
                  {processing ? "Sending Request" : "Request"}
                </button>
                ...
                <div className="text-center">
                  <a
                    href="#"
                    className="text-amber-400 hover:text-amber-300 text-sm transition-colors duration-200"
                    onClick={(e) => updateSection(e, "login")}
                  >
                    Return to Login
                  </a>
                </div>
        ...
        <motion.footer
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, delay: introAniDelay + 0.5 }}
